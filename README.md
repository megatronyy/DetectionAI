...
- **检测统计面板**：右侧可停靠面板，实时累计显示各类别检测次数，支持一键清零
...
- **设置持久化**：自动保存所有用户偏好（阈值、语言、追踪状态、循环播放、类别过滤、统计面板等）
...
├── mainwindow.h/cpp          # 主窗口 UI、控件交互、设置管理、拖放支持
...
- **MainWindow**：纯 UI 层，负责控件交互、设置读写、统计面板更新、检测导出、语言切换、拖放处理、显示渲染后的画面
