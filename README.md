...
基于 Qt 6 + ONNX Runtime + OpenCV 的实时 YOLO11 目标检测桌面应用。支持摄像头、视频文件和 RTSP 网络流输入，实时显示带有检测框、类别标签和追踪 ID 的画面。支持中英文界面切换。
...
- **导出检测结果**：支持 JSON 和 CSV 两种格式导出检测框坐标、类别、置信度
- **检测统计面板**：右侧可停靠面板，实时累计显示各类别检测次数
- **推理耗时显示**：状态栏实时显示每帧推理耗时（毫秒）
- **中英文切换**：一键切换中文/英文界面，所有 UI 文字实时更新
- **键盘快捷键**：Space 暂停/继续、S 截图、O 打开视频、N 网络摄像头、M 切换模型、T 追踪开关、L 循环播放、E 导出检测、F11 全屏、Esc 退出
- **设置持久化**：自动保存窗口大小、阈值、摄像头选择、模型路径、追踪状态、循环播放、语言偏好、类别过滤等用户设置（QSettings）
...
├── inferencethread.h/cpp     # 推理线程（帧循环、推理计时、绘制检测框/追踪框、录制、循环播放）
...
- **InferenceThread**：独立线程运行帧捕获 → YOLO 推理（含计时） → SORT 追踪 → 绘制检测框 → 录制，通过信号槽将 QImage、推理耗时、类别统计传递给 UI 线程
...
- **MainWindow**：纯 UI 层，负责控件交互、设置读写、统计面板更新、检测导出、语言切换、显示渲染后的画面
...
确保 `yolo11n.onnx` 位于工作目录。启动后应用自动加载上次使用的模型并打开默认摄像头，用户偏好（阈值、语言、追踪状态等）自动恢复。
